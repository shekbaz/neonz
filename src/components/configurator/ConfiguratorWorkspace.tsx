 ...
 * Le facteur vise une hauteur de rasterisation constante (~500px) : petites
 * tailles suréchantillonnées, très grandes plafonnées (facteur < 1) pour un
 * temps de squelettisation borné quelle que soit la taille du texte.
 */
...
/**
 * Construit (et met en cache) le tube néon d'un texte : squelette du glyphe,
 * dilaté à l'épaisseur fixe du tube (1cm), comme le tube physique plié par
 * l'artisan. Fidèle à la police uniquement si celle-ci est monoligne (trait
 * d'épaisseur constante) — d'où la liste de polices restreinte aux scripts
 * monolignes et sans-serifs uniformes, voir NEON_FONTS.
 */
  ...
  useEffect(() => {
    ...
    Promise.all(NEON_FONTS.map((font) => document.fonts.load(`32px "${NEON_FONT_FAMILIES[font.id]}"`)))
      .then(() => {
        ...
        // vider le cache force à reconstruire le tube avec la vraie police.
  ...
  function drawCanvas(opts?: { forceLit?: boolean }) {
    ...
    elements.forEach((el) => {
      ...
      if (el.type === "draw") {
        ...
        // Tube à épaisseur fixe (1cm), comme le néon physique : squelette du
        // glyphe dilaté — fidèle à la police car la liste est restreinte aux
        // polices monolignes (voir NEON_FONTS). Pochoir construit à la
        // résolution du backing store (fontSize × 2) puis dessiné à taille
        // logique : pixels alignés sur le canvas 2x, tracé net.
